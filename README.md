...
Vanilla DOM + a tiny `signal()` primitive and a history-API router on the front end; **GenosDB 0.33.4** for data, identity and P2P sync. Built and served with **[Bun](https://bun.sh)**. Installing GenosDB pulls **zero transitive dependencies** — it is the only one.
